@echo off
REM  Databa6ix - load PRODUCER only, into its own DuckDB file.
...
set "DB=%DBDIR%\redshift.producer.duckdb"

REM Enable ONLY the producer for this window.
set "REDSHIFT_CONSUMER_1_ENABLED=false"
set "REDSHIFT_CONSUMER_2_ENABLED=false"
set "REDSHIFT_CONSUMER_3_ENABLED=false"
set "REDSHIFT_ENABLED=true"
set "REDSHIFT_PRODUCER_ENABLED=true"

echo Loading PRODUCER into "%DB%"
...
set "RC=%ERRORLEVEL%"
echo.
if "%RC%"=="0" (echo PRODUCER DONE. When all four windows finish, run MERGE_ALL.cmd.) else (echo PRODUCER stopped code %RC%. Re-run to resume, or open the app once to save Local Credentials.)
pause
exit /b %RC%
