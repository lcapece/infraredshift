@echo off
REM  Databa6ix - load CONSUMER 2 only, into its own DuckDB file.
...
set "DB=%DBDIR%\redshift.consumer_2.duckdb"

REM Enable ONLY Consumer 2 for this window.
...
set "REDSHIFT_CONSUMER_2_ENABLED=true"

echo Loading CONSUMER 2 into "%DB%"
...
set "RC=%ERRORLEVEL%"
echo.
if "%RC%"=="0" (echo CONSUMER 2 DONE. When all four windows finish, run MERGE_ALL.cmd.) else (echo CONSUMER 2 stopped code %RC%. Re-run to resume, or open the app once to save Local Credentials.)
pause
exit /b %RC%
