@echo off
REM  Databa6ix - load CONSUMER 1 only, into its own DuckDB file.
...
set "DB=%DBDIR%\redshift.consumer_1.duckdb"

REM Enable ONLY Consumer 1 for this window.
...
set "REDSHIFT_CONSUMER_1_ENABLED=true"

echo Loading CONSUMER 1 into "%DB%"
...
set "RC=%ERRORLEVEL%"
echo.
if "%RC%"=="0" (echo CONSUMER 1 DONE. When all four windows finish, run MERGE_ALL.cmd.) else (echo CONSUMER 1 stopped code %RC%. Re-run to resume, or open the app once to save Local Credentials.)
pause
exit /b %RC%
